...
//   const sendWhatsApp = () => {
//     const text = `*🚕 TAXI BOOKING SUMMARY*%0A---%0A*Trip:* ${formData.tripType}%0A*Car:* ${config.name}%0A*From:* ${originRef.current.value}%0A*To:* ${destRef.current.value}%0A*Dates:* ${formData.startDate} ${formData.endDate ? 'to ' + formData.endDate : ''}%0A*Duration:* ${days} Day(s)%0A*Total KM:* ${formData.tripType === 'ROUND_TRIP' ? distance * 2 : distance}km%0A---%0A*Fare Breakdown:*%0A- Min KM (${minKmApplied}km): ₹${minKmApplied * config.extra}%0A- Extra KM (${extraKm}km): ₹${extraFare}%0A- Driver Bata (${days} days): ₹${totalBata}%0A---%0A*TOTAL FARE: ₹${totalAmount}*`;
//     window.open(`https://wa.me/918760212345?text=${text}`, "_blank");
//   };

//   if (!isLoaded) return <div className="p-10 text-center font-black text-[#22C55E]">INITIALISING FLEET...</div>;

//   return (
//     <div className="w-full max-w-lg mx-auto">
//       <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-1 shadow-2xl border-b-8 border-[#22C55E]">
...
//                 className={`flex-1 py-3 rounded-xl text-xs font-black transition-all ${formData.tripType === t ? "bg-black text-[#22C55E]" : "text-slate-500"}`}
//               >
//                 {t.replace("_", " ")}
//               </button>
//             ))}
...
//               <input type="text" placeholder="Name" className="input-field-new" onChange={(e) => setFormData({...formData, name: e.target.value})} />
//               <input type="tel" placeholder="Phone" className="input-field-new" onChange={(e) => setFormData({...formData, phone: e.target.value})} />
//             </div>

//             {/* Locations */}
//             <Autocomplete><input ref={originRef} placeholder="Pickup City" className="input-field-new" /></Autocomplete>
//             <Autocomplete><input ref={destRef} placeholder="Drop City" className="input-field-new" /></Autocomplete>

//             {/* Dates */}
//             <div className={`grid ${formData.tripType === 'ROUND_TRIP' ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
//               <div className="relative">
//                 <span className="absolute left-3 top-1 text-[8px] font-bold text-slate-400 uppercase">Start Date</span>
//                 <input type="date" className="input-field-date" onChange={(e) => setFormData({...formData, startDate: e.target.value})} />
//               </div>
//               {formData.tripType === "ROUND_TRIP" && (
//                 <div className="relative">
//                    <span className="absolute left-3 top-1 text-[8px] font-bold text-slate-400 uppercase">End Date</span>
//                    <input type="date" className="input-field-date" onChange={(e) => setFormData({...formData, endDate: e.target.value})} />
//                 </div>
//               )}
//             </div>
...
//                   className={`py-3 text-black rounded-xl border-2 flex flex-col items-center transition-all ${formData.carType === c ? "border-[#22C55E] bg-green-50" : "border-slate-100"}`}
//                 >
//                   <span className="text-[10px] font-black">{c}</span>
//                   <span className="text-[8px] font-bold text-slate-500">₹{PRICING_DATA[formData.tripType][c].extra}/KM</span>
//                 </button>
//               ))}
//             </div>

//             <button onClick={handleCalculate} className="w-full bg-black text-[#22C55E] py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-[#22C55E] hover:text-white transition-all">
//               Calculate Fare
//             </button>
//           </div>
//         </div>
...
//             <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="relative bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl">
//               <div className="bg-[#22C55E] p-8 text-black">
//                 <div className="flex justify-between items-start">
//                   <h4 className="text-4xl font-black italic tracking-tighter leading-none">FARE<br/>SUMMARY</h4>
//                   <X className="cursor-pointer" onClick={() => setShowReceipt(false)} />
//                 </div>
//               </div>
...
//                     <CalendarDays className="text-[#22C55E]" />
//                     <div>
//                       <p className="text-[10px] font-black uppercase text-slate-500">Total Duration</p>
//                       <p className="font-bold text-slate-600">{days} Day(s)</p>
//                     </div>
//                   </div>
//                   <div className="text-right">
//                     <p className="text-[10px] font-black uppercase text-slate-500">Total KM</p>
//                     <p className="font-bold text-slate-600">{formData.tripType === 'ROUND_TRIP' ? distance * 2 : distance} KM</p>
//                   </div>
//                 </div>

//                 <div className="space-y-3">
//                   <div className="flex justify-between text-sm font-bold uppercase italic tracking-tight">
//                     <span className="text-slate-500">Min. Commitment ({minKmApplied}km)</span>
//                     <span className="text-slate-600">₹{minKmApplied * config.extra}</span>
//                   </div>
//                   <div className="flex justify-between text-sm font-bold uppercase italic tracking-tight">
//                     <span className="text-slate-500">Extra Usage ({extraKm}km)</span>
//                     <span className="text-slate-600">₹{extraFare}</span>
//                   </div>
//                   <div className="flex justify-between text-sm font-bold uppercase italic tracking-tight border-b pb-3">
//                     <span className="text-slate-500">Driver Bata ({days} days)</span>
//                     <span className="text-slate-600">₹{totalBata}</span>
//                   </div>
//                   <div className="flex justify-between items-center pt-2">
//                     <span className="text-xl font-black uppercase italic tracking-tighter text-slate-500">Total Estimate</span>
//                     <span className="text-3xl font-black text-[#22C55E]">₹{totalAmount}</span>
//                   </div>
//                 </div>

//                 <button onClick={sendWhatsApp} className="w-full bg-[#25D366] text-white py-5 rounded-2xl font-black flex items-center justify-center gap-3 shadow-lg hover:shadow-green-500/20 transition-all">
//                   <MessageCircle fill="white" size={24} /> CONFIRM VIA WHATSAPP
//                 </button>
//               </div>
//             </motion.div>
//           </div>
...
//           color: black;
//         }
//         .input-field-new:focus { border-color: #22c55e; background: white; }
//         .input-field-date {
//           width: 100%;
//           background: #f1f5f9;
//           border: 2px solid transparent;
